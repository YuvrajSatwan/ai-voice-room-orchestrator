...
async def test_s1_plain_question_is_answered_by_one_bot_in_voice_and_chat() -> None:
    ...
    assert bots[Persona.DOST].chat == ["Roxstar AI Dost reply 1."]
...
async def test_s3_follow_up_prompt_carries_the_earlier_answer() -> None:
    ...
    assert "Roxstar AI Dost: Roxstar AI Dost reply 1." in prompt
...
async def test_s4_second_user_follow_up_goes_to_the_same_bot_with_shared_context() -> None:
    brain, _, _, llm = make_brain()
    await brain.handle_turn(speaker="Rahul", text="AI Sathi, AI kya hota hai?", channel=VOICE)
    plan = await brain.handle_turn(speaker="Priya", text="Thoda aur simple batao", channel=VOICE)
    assert plan.responders == (Persona.SATHI,)
    assert "Rahul: AI Sathi, AI kya hota hai?" in llm.last_prompt()
...
async def test_s7_two_bot_request_runs_in_order_without_overlap() -> None:
    ...
    assert "Roxstar AI Dost reply 1." in sathi_prompt  # Sathi builds on Dost's answer
...
async def test_two_bot_plan_is_not_split_by_another_users_turn() -> None:
    ...
    both = asyncio.create_task(
        brain.handle_turn(
            speaker="Rahul", text="Dost answer karo, phir Sathi example", channel=VOICE
        )
    )
    await asyncio.sleep(0.01)
    other = asyncio.create_task(
        brain.handle_turn(speaker="Priya", text="AI Dost, cloud kya hai?", channel=TEXT)
    )
...
async def test_simultaneous_turns_from_two_users_never_overlap() -> None:
    ...
    await asyncio.gather(
        brain.handle_turn(speaker="Rahul", text="AI Dost, AI kya hai?", channel=VOICE),
        brain.handle_turn(speaker="Priya", text="AI Sathi, cloud kya hai?", channel=VOICE),
    )
...
async def test_llm_failure_speaks_a_fallback_and_the_room_keeps_working() -> None:
    ...
    llm.fail = False
    await brain.handle_turn(speaker="Rahul", text="AI kya hai?", channel=VOICE)
    assert bots[Persona.DOST].chat[-1].startswith("Roxstar AI Dost reply")
...
async def test_after_an_apology_the_follow_up_goes_back_to_the_same_bot() -> None:
    ...
    await brain.handle_turn(speaker="Rahul", text="AI Sathi, AI kya hota hai?", channel=TEXT)
    llm.fail = False
...
async def test_follow_up_sent_while_the_bot_is_still_talking_goes_to_that_bot() -> None:
    ...
    first = asyncio.create_task(
        brain.handle_turn(speaker="Rahul", text="AI Sathi, AI kya hai?", channel=TEXT)
    )
    await asyncio.sleep(0.05)  # Sathi is mid-sentence
...
async def test_llm_failure_cancels_the_rest_of_a_two_bot_plan() -> None:
    ...
    await brain.handle_turn(speaker="Rahul", text="Dost bolo, phir Sathi", channel=VOICE)
    assert bots[Persona.SATHI].chat == []
...
async def test_submit_turn_logs_crashes_instead_of_raising(caplog) -> None:
    ...
    await asyncio.wait({brain.submit_turn(speaker="Rahul", text="hi Dost", channel=TEXT)})
...
async def test_demo_switch_fails_the_next_llm_call_once_through_the_real_path() -> None:
    ...
    await brain.handle_turn(speaker="Rahul", text="AI kya hai?", channel=VOICE)
    assert bots[Persona.DOST].chat[-1].startswith("Roxstar AI Dost reply")  # one-shot
...
async def test_a_follow_up_prompt_quotes_the_answer_it_follows_up_on() -> None:
    ...
    assert (
        "This is a follow-up to the last answer, by Roxstar AI Dost: "
        '"Roxstar AI Dost reply 2."' in llm.last_prompt()
    )
